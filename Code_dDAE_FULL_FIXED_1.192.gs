...
function handlePulizie_(e, method) {
  ...
  ensureHeaders_(sh, ["id","data","stanza","MAT","SIN","FED","TDO","TFA","TBI","TAP","TPI","createdAt","updatedAt"]);
...
function handleLavanderia_(e, method) {
  ...
  // colonne minime
  ensureHeaders_(sh, ["id","startDate","endDate","MAT","SIN","FED","TDO","TFA","TBI","TAP","TPI","createdAt","updatedAt"]);
  ensureHeaders_(shPul, ["id","data","stanza","MAT","SIN","FED","TDO","TFA","TBI","TAP","TPI","createdAt","updatedAt"]);
...
function handleOperatori_(e, method) {
  ...
  if (method === "GET") {
    const qDate = isoDatePulizie_(e.parameter.data || e.parameter.date || "");
    const all = readAll_(sh);
    if (!qDate) return jsonOk_({ rows: all.rows });
    const out = all.rows.filter(r => isoDatePulizie_(r.data || r.date || "") === qDate);
  ...
  const body = parseBody_(e);
  const date = isoDatePulizie_(body.data || body.date || "");
  ...
  if (!headers.length || headers.every(h => !h)) {
    const std = ["id","data","operatore","ore","benzina_euro","note","createdAt","updatedAt"];
    sh.clearContents();
    sh.appendRow(std);
  ...
  // Mappa esistenti per upsert
  const existing = new Map();
  for (let r = 1; r < values.length; r++) {
    const row = values[r] || [];
    const d = isoDatePulizie_(row[cData] || "");
    const o = norm(row[cOp]);
    if (d && o) existing.set(d + "||" + o, r + 1); // 1-based row index
  }
  ...
  items.forEach(it => {
    ...
    const key = date + "||" + norm(operatore);
    const rowIndex = existing.get(key);

    if (rowIndex) {
      // update
      if (cOre >= 0) sh.getRange(rowIndex, cOre + 1).setValue(ore);
      if (cBenz >= 0) sh.getRange(rowIndex, cBenz + 1).setValue(benz === "" ? 2 : benz);
      if (cUpdatedAt >= 0) sh.getRange(rowIndex, cUpdatedAt + 1).setValue(now);
      saved++;
      return;
    }
    ...
    sh.appendRow(newRow);
    existing.set(key, sh.getLastRow());
    saved++;
  });
